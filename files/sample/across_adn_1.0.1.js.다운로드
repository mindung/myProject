			...
			if(adn_health_flag=="ok"){
				if(typeof p !== 'undefined'){
					...
					try{
						...
						if(a=="" && ui==adn_ui){
							fn_across_adn_setCookie("_adn_ck_", encodeURIComponent(adn_ui + "|" + adn_ci + "|" + adn_gi + "|" + adn_ii + "|" + adn_pi + "|" + adn_ki + "|" + adn_wd), 31);
						}
					}catch(e){}	
					...
					if(ut.toLowerCase()=="home"){
						...
						{
							var p_url="https://kn.acrosspf.com/adn_ga_in_order_1.0.2.ad?ad_ids="+across_rb_adn_uid+"&u="+ui+"&o="+uo+"&p="+up+"&pi="+across_adn_ad_trk_parse[3]["pi"]+"&ci="+across_adn_ad_trk_parse[5]["ci"]+"&gi="+across_adn_ad_trk_parse[6]["gi"]+"&ii="+across_adn_ad_trk_parse[7]["ii"]+"&i="+encodeURIComponent(purchase_items_data)+"&r="+Math.floor(Math.random()*99999999999);
							var across_obj_id = fn_across_adn_obj();
							fn_across_adn_frm_send(across_obj_id, p_url);
							/*jQuery.ajax({url: p_url,dataType: 'jsonp',success: function(data){}}); */
						}
